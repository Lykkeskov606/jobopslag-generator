  ...
  async function generateProfile() {
    ...
    try {
      ...
      if (err.response?.status === 422) {
        setError(da ? 'Indhold kan ikke genereres pga. indholdspolitik.' : 'Content cannot be generated due to content policy.');
      } else {
        setError(t('tier2.step8ErrorProfile'));
      }
    } finally {
      setGenerating(false);
    }
        ...
        <div style={{ marginBottom: 32 }}>
          ...
          {error && !generating && (
            <div style={{ marginBottom: 16 }}>
              <p style={{ color: 'var(--error, #c0392b)', marginBottom: 8 }}>{error}</p>
          ...
          {profileContent && (
            <div>
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: '24px 28px', marginBottom: 12, lineHeight: 1.7, fontSize: 14, whiteSpace: 'pre-wrap' }}>
                {profileContent}
              </div>
              <button type="button" className="btn btn-ghost" onClick={generateProfile} disabled={generating}>
        ...
        <div className="actionbar">
          <div className="actionbar-inner">
            ...
            <button
              ...
              className="btn btn-primary btn-lg"
              onClick={onComplete}
              disabled={!profileContent || generating}
            >
              {t('tier2.continue')}
              <span className="arrow">→</span>
            </button>
