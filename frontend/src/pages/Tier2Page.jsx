...
function Step7JobPosting({ state, onBack, onComplete, onSkipCompleteness, t, project, da }) {
  ...
  const [subStep, setSubStep]               = useState('restoring');
  ...
  // to this step must show the existing variants (and any saved final edit),
  // not trigger a fresh generation. Only auto-generate when nothing exists yet.
  useEffect(() => {
    if (hasRestored.current) return;
    hasRestored.current = true;
    (async () => {
      try {
        const { data } = await api.get(`/tier2/${project.id}/generated`);
        if (data.variant_a || data.variant_b) {
          setVariantA(data.variant_a || '');
          setVariantB(data.variant_b || '');
          setGenerationBatch(data.generation_batch || null);
          if (data.selected_variant && data.final_content) {
      ...
      } catch { /* fall through to the normal first-visit flow */ }
      if (step7Missing.length === 0) generate([]);
  ...
  if (subStep === 'restoring' || subStep === 'generating') {
    return (
      <div className="app">
        <TopBar active="projects" />
        <main>
...
function Step8GenerateOutputs({ state, project, onBack, onComplete, t, da }) {
  ...
  // Rehydrate previously generated documents before spending AI calls —
  // regeneration only happens via the explicit buttons below.
  useEffect(() => {
    if (hasRestored.current) return;
    hasRestored.current = true;
    (async () => {
      ...
      try {
        ...
        if (data.candidate_profile) {
          setProfileContent(data.candidate_profile);
          restoredProfile = true;
        }
        if (Array.isArray(data.interview_guide_items) && data.interview_guide_items.length) {
          setGuideItems(data.interview_guide_items);
        }
      } catch { /* fall through to the normal first-visit flow */ }
      setRestoring(false);
      if (!restoredProfile) generateProfile();
    })();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  if (restoring || (generatingProfile && !profileContent)) {
    return (
      <div className="app">
        <TopBar active="projects" />
        <main>
